...
namespace PlcTableMonitor
{
    public partial class FrmMain : Form
    {
        ...
        public FrmMain()
        {
            ...
            InitDb();
            InitGrid();
            InitExceptionRecordDataGridView();
            InitTimer();
        ...
        /// 初始化异常记录DataGridView
        /// </summary>
        private void InitExceptionRecordDataGridView()
        {
            // 清除现有列（防止重复初始化）
            dataGridViewExceptionRecords.Columns.Clear();
            // 添加列
            dataGridViewExceptionRecords.Columns.Add("Id", "ID");
            dataGridViewExceptionRecords.Columns.Add("TableName", "表名");
            dataGridViewExceptionRecords.Columns.Add("ExceptionType", "异常类型");
            dataGridViewExceptionRecords.Columns.Add("ExceptionDetail", "详细信息");
            dataGridViewExceptionRecords.Columns.Add("RecordTime", "创建时间");
            dataGridViewExceptionRecords.Columns.Add("AdditionalInfo", "附加信息");
            dataGridViewExceptionRecords.Columns.Add("IsRecovered", "是否恢复");
            dataGridViewExceptionRecords.Columns.Add("PreviousRecordId", "前回ID");

            // 禁止手动添加行
            dataGridViewExceptionRecords.AllowUserToAddRows = false;
            // 禁止编辑单元格
            dataGridViewExceptionRecords.ReadOnly = true;
            // 禁止排序（可选）
            foreach (DataGridViewColumn column in dataGridViewExceptionRecords.Columns)
            {
                column.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
            ...
            // 允许单元格自动换行
            dataGridViewExceptionRecords.DefaultCellStyle.WrapMode = DataGridViewTriState.True;

            // 自动调整行高（根据内容）
            dataGridViewExceptionRecords.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;

            // 设置列宽模式（保持 Fill 模式，但调整 FillWeight）
            dataGridViewExceptionRecords.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            // ✅ 启用行头显示（行号）
            dataGridViewExceptionRecords.RowHeadersVisible = true;
            // ✅ 订阅 RowPostPaint 事件，绘制行号
            dataGridViewExceptionRecords.RowPostPaint += DataGridView_RowPostPaint;
        }
        /// <summary>
        /// 刷新监控日志 数据源
        /// </summary>
        private void LoadExceptionRecords()
        {
            try
            {
                // 显示加载状态
                ...
                // 清空现有数据源
                dataGridViewExceptionRecords.Rows.Clear(); // 先清空

                // 1. 从数据库查询数据（SQLSugar示例）
                List<ExceptionRecord> records = SqlSugarContext.Instance.GetDbClient(DatabaseType.SQLite)
                    .Queryable<ExceptionRecord>()
                ...
                {
                    int rowIndex = dataGridViewExceptionRecords.Rows.Add(
                        record.Id,
                        record.TableName,
                        record.ExceptionType,
                        record.ExceptionDetail,
                        record.RecordTime?.ToString("yyyy-MM-dd HH:mm:ss.fff"),
                        record.AdditionalInfo,
                        record.IsRecovered ? "是" : "否",
                        record.PreviousRecordId);
                    // 设置行样式
                    if (record.IsRecovered)
                    {
                        dataGridViewExceptionRecords.Rows[rowIndex].DefaultCellStyle.BackColor = Color.LightGreen;
                    }
                    else
                    {
                        dataGridViewExceptionRecords.Rows[rowIndex].DefaultCellStyle.BackColor = Color.Khaki;
                    }
                }

                // 显示提示
                MessageBox.Show($"加载完成，共 {records.Count} 条数据", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
